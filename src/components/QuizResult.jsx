...
const QuizResult = ({ questions, selectedAnswers, amount }) => {
      ...
      <div className="w-[80%] h-full flex justify-center flex-col space-y-3 p-4 bg-neutral-600/20 border border-neutral-100/20 backdrop-blur rounded-xl overflow-hidden">
        ...
        <div className="w-full flex items-center gap-6 border-b pb-4 border-neutral-100/20">
          ...
          {notAttempted.length > 0 && (
            <div className="flex items-center my-0 gap-x-2 bg-red-800/60 px-3 pb-0.5 rounded-lg">
              Total Not Attempted: <span className="font-bold">{notAttempted.length}</span>
            </div>
          )}

          {notAttempted.length > 0 && (
            <div className="flex items-center my-0 gap-x-2 bg-neutral-800/60 px-3 pb-0.5 rounded-lg">
              Not Attempted Questions: <span className="font-bold">{notAttempted.join(', ')}</span>
            </div>
          )}
        </div>
        ...
        <div className="score mt-4">
          <p>Your Score: {correctAnswers.length} out of {questions.length}</p>
        </div>
      </div>
    </div>
  );
